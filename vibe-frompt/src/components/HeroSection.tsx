...
export default function HeroSection() {
    ...
    return (
        <section ref={heroRef} style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', padding: '120px 24px 80px' }}>
            {/* Animated background orbs */}
            <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
                <motion.div style={{ position: 'absolute', top: '20%', left: '10%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,245,255,0.08) 0%, transparent 70%)', filter: 'blur(40px)', y }} />
                <motion.div style={{ position: 'absolute', top: '40%', right: '5%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,0,204,0.07) 0%, transparent 70%)', filter: 'blur(40px)', y: useTransform(scrollYProgress, [0, 1], [0, -60]) }} />
            ...
            <div className="container" style={{ position: 'relative', zIndex: 2, textAlign: 'center', maxWidth: 900 }}>
                {/* Badge */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 32 }}>
                ...
                {/* Subtitle */}
                <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} style={{ fontSize: 'clamp(18px, 2.5vw, 24px)', color: 'rgba(255,255,255,0.7)', marginBottom: 12, fontWeight: 300, lineHeight: 1.5 }}>
                    Vibe Your Prompt – AI Hiểu Ý Bạn Chuẩn{' '}
                    <span style={{ color: '#00f5ff', fontWeight: 600 }}>100%</span>
                </motion.p>

                {/* Stats */}
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} style={{ display: 'flex', justifyContent: 'center', gap: 32, marginBottom: 48, flexWrap: 'wrap' }}>
                    {[
                ...
                <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.35 }} style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: '20px 28px', marginBottom: 44, backdropFilter: 'blur(20px)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, textAlign: 'left', maxWidth: 680, margin: '0 auto 44px' }}>
                    ...
                    <div style={{ borderLeft: '1px solid rgba(0,245,255,0.15)', paddingLeft: 20 }}>
                        ...
                        <motion.div key={demoLine} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', minHeight: 24, lineHeight: 1.5 }}>
                ...
                {/* CTA */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
